...
norms = ["bad","gray","log"]
methods = ["SIFT","ORB","SAR_SIFT","OS_SIFT","RIFT"]
grd = [10,1,0.35,0.35]

ilorazy = [grd[0]/0.35,grd[1]/0.35,grd[2]/0.35,grd[3]/0.35]

for m in methods:
        with open(f"report_{m}/report_{m}_2rmse_3_MASK_HOMO.html", "w") as raport:
            numerek = 0
            for d in datas:
                ...
                for j,s in enumerate(scales):
                    for n in norms:
                        ...
                        try:
                            M_ref, mask = cv2.findHomography(ptk_CAP_ref, ptk_PNEO_ref)

                            # Macierz powstała w skutek dopasowania automatyczną metodą
                            if len(ptk_CAP)>0:
                                M_nowa, mask_0 = cv2.findHomography(ptk_CAP, ptk_PNEO, )
                            else:
                                raport.write(f"<h3 style='color:red;'>not enought points</h3>")
                                continue
                        except:
                        ...
                        CMR,treshold,blad,mask = RMSE.calculate_CMR_mask_new_Homo(M_ref,ptk_CAP_check,ptk_PNEO_check,ptk_CAP,ptk_PNEO)
                        #print(ptk_CAP-ptk_PNEO)
                        rmse,bled = RMSE.calculate_RMSE_Homo(M_nowa,ptk_CAP_check,ptk_PNEO_check)
                        rmse_na_pw,bled = RMSE.calculate_RMSE_Homo(M_nowa,ptk_CAP,ptk_PNEO)
                        print(rmse*grd[j])
                        print(CMR)
                        ...
                        h,w = img2.shape
                        image = cv2.warpPerspective(img1,M_nowa,(h,w))
