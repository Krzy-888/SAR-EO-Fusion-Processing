...
for m in methods:
        with open(f"report_{m}/report_{m}_2rmse_3_piks.html", "w") as raport:
            numerek = 0
            for d in datas:
                ...
                for j,s in enumerate(scales):
                    for n in norms:
                        ...
                        CMR,treshold,blad,mask = RMSE.calculate_CMR_mask_piks(M_ref,ptk_CAP,ptk_PNEO,grd[j])
                        ...
                        raport.write(f"<h3>RMSE [m]:  {rmse_na_pw*grd[j]}, RMSE k [m]:  {rmse*grd[j]}, CMR[%]:  {CMR}, treshold:  {treshold}, T: {T[0]},  NOP:  {n_o_p}</h3>")
                        h,w = img2.shape
                        ...
                        plt.tight_layout()
                        plt.savefig(f"report_{m}/SAR_{d}_SUB_{s}m_{n}-EO_{d}_SUB_{s}m_gray_bad_3_MASK_pik.png", dpi=300,)
                        numerek+=1
                        raport.write(f"<img src='SAR_{d}_SUB_{s}m_{n}-EO_{d}_SUB_{s}m_gray_bad_3_MASK_pik.png'/>")
                print(f"End {d}")
